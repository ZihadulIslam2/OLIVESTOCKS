...
export default function AddStockSearch({
  onStockAdded,
  refetch,
}: AddStockSearchProps) {
  ...
  const { mutate: addHolding, isPending: isAddingHolding } = useMutation({
    mutationFn: async (data: AddHoldingData) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to add stock to portfolio."
        );
      }
  ...
  const { mutateAsync } = useMutation<unknown, unknown, StockResult>({
  ...
  const handleWatchlist = async (rowData: any) => {
    ...
    <div className="relative w-full" ref={searchRef}>
      <div className="relative">
        <input
          ...
          placeholder={
            pathName === "/watchlist"
              ? "Search any Stock to add wathclist"
              : "Search any Stock to add to your Portfolio...."
          }
